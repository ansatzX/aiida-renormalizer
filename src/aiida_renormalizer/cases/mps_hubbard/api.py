...
def record_random_state(*, quantum_number, bond_dimension, percent):
    if bond_dimension < 1 or not 0 <= percent <= 1:
        raise ValueError("invalid random-state bond dimension or sector sampling fraction")
    return _record_section(
        orm.Str(
            f"state = Mps.random(model, qntot={literal(quantum_number)}, "
            f"m_max={bond_dimension!r}, percent={percent!r})\n"
        )
    )


def record_dmrg(*, optimize_config, copy_initial_state):
    source = configuration_source(optimize_config, "optimize_config")
    source += (
        "\nstate, dmrg_result = optimize_state(state, mpo, "
        "optimize_config=optimize_config, "
        f"copy_initial_state={copy_initial_state!r})\n"
    )
    source += "result['dmrg'] = dmrg_result\n"
    return _record_section(orm.Str(source))


def record_imaginary_time(*, evolve_config, dt, max_steps, energy_change_atol, normalize):
    if complex(dt).real != 0 or complex(dt).imag >= 0:
        raise ValueError("imaginary-time dt must be negative imaginary")
    if max_steps < 1 or energy_change_atol <= 0:
        raise ValueError("positive step cap and energy tolerance required")
    evolve_config.check_valid_dt(dt)
    source = configuration_source(evolve_config, "evolve_config")
    source += (
        "\nstate, imaginary_result = imaginary_time_segment(state, mpo, "
        "evolve_config=evolve_config, "
        f"dt={dt!r}, max_steps={max_steps!r}, energy_change_atol={energy_change_atol!r}, "
        f"normalize={normalize!r})\n"
        "result['imaginary_time'] = imaginary_result\n"
    )
    return _record_section(orm.Str(source))
...
def _assemble(**sections) -> orm.Str:
    ...
    source = (
        runtime
        + "\n\ndef main(output_file='hubbard_result.json'):\n"
        + body
        + "\n\nif __name__ == '__main__':\n"
        "    main(Path(__file__).with_name('hubbard_result.json'))\n"
    )

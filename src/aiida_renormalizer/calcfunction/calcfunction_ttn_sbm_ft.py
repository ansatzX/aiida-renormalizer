"""TTN SBM FT case-scoped calcfunctions."""
    ...
    stages = [
        {
            ...
            "script": _render_stage_script(
                "ttn_sbm_ft_write_generated_script_stage.py.jinja",
        ...
        },
        {
            ...
            "script": _render_stage_script(
                "ttn_sbm_ft_compile_generated_script_stage.py.jinja",
...
@calcfunction
def known_parameters_from_input_model_calc(
    input_params: orm.Dict,
    model_params: orm.Dict,
    calc_params: orm.Dict,
    system_hamiltonian_terms: orm.List,
) -> orm.Dict:
    return orm.Dict(
        dict={
            **dict(input_params.get_dict()),
            **dict(model_params.get_dict()),
            **dict(calc_params.get_dict()),
            "system_hamiltonian_terms": list(system_hamiltonian_terms.get_list()),
        }
    )


@calcfunction
def spectral_density_parameters_from_input(known_parameters: orm.Dict) -> orm.Dict:
    params = dict(known_parameters.get_dict())
    required = ["ita", "omega_c", "beta", "upper_limit"]
    missing = [k for k in required if k not in params]
    if missing:
        raise ValueError(f"missing keys for spectral density parameters: {', '.join(missing)}")
    return orm.Dict(
        dict={
            "ita": float(params["ita"]),
...
@calcfunction
def renormalize_and_discretize_modes(
    known_parameters: orm.Dict,
    spectral_density_parameters: orm.Dict,
) -> orm.Dict:
    import numpy as np
    from renormalizer.sbm import ColeDavidsonSDF

    params = dict(known_parameters.get_dict())
    sdf_params = dict(spectral_density_parameters.get_dict())
    required = ["n_modes", "raw_delta"]
    missing = [k for k in required if k not in params]
    if missing:
        raise ValueError(f"missing keys for renormalization/discretization: {', '.join(missing)}")

    sdf = ColeDavidsonSDF(
        float(sdf_params["ita"]),
        float(sdf_params["omega_c"]),
        float(sdf_params["beta"]),
        float(sdf_params["upper_limit"]),
    )
    omega_k, c_j2 = sdf.Wang1(int(params["n_modes"]))
    renormalization_constant = float(sdf.reno(float(omega_k[-1])))
    delta_eff = float(float(params["raw_delta"]) * renormalization_constant)

    return orm.Dict(
        dict={
            "omega_k": np.asarray(omega_k, dtype=float).tolist(),
            "c_j2": np.asarray(c_j2, dtype=float).tolist(),
            "renormalization_constant": renormalization_constant,
            "delta_eff": delta_eff,
        }
    )
...
def _render_case_script_from_specs_payload(
    *,
    template_name: str,
    script_name: str,
    input_params: orm.Dict,
...
@calcfunction
def ttn_sbm_ft_script_from_spec(
    input_params: orm.Dict,
    model_params: orm.Dict,
    calc_params: orm.Dict,
    op_spec,
    ...
    phonon_labels = [
        int(item["nbas"]) > int(model_params.get_dict()["m_max"])
        for item in basis_spec.as_list()
        if item["kind"] == "sho"
    ]
    return orm.Dict(
        dict=_render_case_script_from_specs_payload(
            template_name="example_ttn_sbm_ft_from_spec_single_file.py.jinja",
            script_name="ttn_sbm_ft_generated.py",
            ...
            real_run=real_run,
            spin_dof="spin",
            extra_context={"contract_label_literal": repr(phonon_labels)},
        )
    )
...
gather_known_parameters = known_parameters_from_input_model_calc
extract_spectral_density_parameters = spectral_density_parameters_from_input
build_environment_modes = renormalize_and_discretize_modes
define_hamiltonian_terms = define_op_spec
define_basis = define_basis_spec
build_ttn_script = ttn_sbm_ft_script_from_spec

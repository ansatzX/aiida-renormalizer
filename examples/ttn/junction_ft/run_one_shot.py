#!/usr/bin/env python
"""TTN junction finite-temperature script-generation example."""
...
from aiida_renormalizer.calcfunction.calcfunction_ttn_junction_ft import (
...
def main() -> None:
    ...
    c = numpy.sqrt(c2)
    reno = sdf.reno(w[-1])
    temperature_beta = Quantity(TEMPERATURE_K, "K").to_beta()
    ...
    for mode, energy in mode_with_energy:
        ...
        ordered_electrode_dofs.append(f"{mode}_p")
        ordered_electrode_dofs.append(f"{mode}_q")

    # Build model: full Hamiltonian term list.
    theta_array = numpy.arctanh(numpy.exp(-w * temperature_beta / 2))
    ...
    for mode, energy in mode_with_energy:
        ...
        hamiltonian_terms_py.extend(
            [
                ["+ -", dof_p, float(energy + mu), 0],
                ["+ -", dof_q, -float(energy + mu), 0],
            ]
        )

        v2 = alpha_e**2 / beta_e**2 * numpy.sqrt(4 * beta_e**2 - (energy + mu) ** 2) / 2 / numpy.pi / rho_e
        coupling = float(numpy.sqrt(v2))
        theta = float(numpy.arctan(numpy.exp(-temperature_beta * energy / 2)))

        for dof, factor in [
            (dof_p, coupling * numpy.cos(theta)),
            (dof_q, coupling * numpy.sin(theta)),
        ]:
            idx = ordered_electrode_dofs.index(dof)
            if idx < s_idx:
            ...
            if dof.endswith("_p"):
                hamiltonian_terms_py.extend(
                    [
                        ["+ " + "Z " * len(z_dofs) + "-", [dof] + z_dofs + ["s"], float(factor), [0] * (len(z_dofs) + 2)],
                        ["- " + "Z " * len(z_dofs) + "+", [dof] + z_dofs + ["s"], float(factor), [0] * (len(z_dofs) + 2)],
                    ]
                )
            else:
    ...
    for imode, omega in enumerate(w):
        hamiltonian_terms_py.extend(
            [
                ["p^2", f"v_{imode}_p", 0.5, 0],
                ["x^2", f"v_{imode}_p", 0.5 * float(omega) ** 2, 0],
                ["p^2", f"v_{imode}_q", -0.5, 0],
                ["x^2", f"v_{imode}_q", -0.5 * float(omega) ** 2, 0],
            ]
        )
    ...
    for imode, coupling in enumerate(c):
        ...
        op_p = sys_op * Op(
            "x",
            f"v_{imode}_p",
            factor=2 * float(coupling) * numpy.cosh(theta_array[imode]),
            qn=[0],
        )
        op_q = sys_op * Op(
            "x",
            f"v_{imode}_q",
            factor=2 * float(coupling) * numpy.sinh(theta_array[imode]),
            qn=[0],
        )

        for product in (op_p, op_q):
            ...
            for op in terms:
                ...
                if abs(factor_c.imag) > 1e-12:
                    raise ValueError(f"junction_ft expects real primitive terms, got {factor}")
    ...
    nbas = numpy.maximum(16 * c2 / w**3 * numpy.cosh(theta_array) ** 2, numpy.ones(len(w)) * 4)
    nbas = numpy.minimum(nbas, numpy.ones(len(w)) * 512)
    nbas = numpy.round(nbas).astype(int)
    for imode, omega in enumerate(w):
        basis_py.append(["sho", f"v_{imode}_p", float(omega), int(nbas[imode])])
        basis_py.append(["sho", f"v_{imode}_q", float(omega), int(nbas[imode])])

    contract_label = numpy.array([[nbas > M_MAX], [nbas > M_MAX]], dtype=bool).T.ravel().tolist()
    topology_py = {
        "schema": "topology_v1",
        "subtrees": [
            {
            ...
            },
            {
                "subtree_id": "phonon",
                "builder": "binary_mctdh",
                "basis_dofs": [f"v_{imode}_{branch}" for imode in range(N_PH_MODE) for branch in ("p", "q")],

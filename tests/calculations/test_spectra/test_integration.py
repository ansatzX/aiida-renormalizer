...
def test_import_all_calculations():
    """Test that all CalcJob classes can be imported."""
    from aiida_renormalizer.calculations.spectra import (
        SpectraZeroTCalcJob,
    ...
    assert SpectraZeroTCalcJob is not None
    assert SpectraFiniteTCalcJob is not None
    assert KuboCalcJob is not None
    assert CorrectionVectorCalcJob is not None
    assert ChargeDiffusionCalcJob is not None
    assert SpectralFunctionCalcJob is not None


def test_calcjob_inheritance():
    """Test that all CalcJobs inherit from RenoBaseCalcJob."""
    ...
    for calc_class in calc_classes:
        assert issubclass(calc_class, RenoBaseCalcJob), \
            f"{calc_class.__name__} doesn't inherit from RenoBaseCalcJob"
